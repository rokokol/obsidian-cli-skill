...
scripts, workflows, docs, the changelog — and needs actionlint, shellcheck and shfmt.
behaviour drives obsi.sh against a fake CLI and needs only bash and node, so it can run
under the bash 3.2 that macOS ships, which is what obsi.sh claims to run on; with
CHECK_BASH32=1 it first proves that this bash is that 3.2. all, the default, is both.
Every tool comes from the flake, never from PATH's luck

  nix develop -c ./check.sh
  nix shell --inputs-from . nixpkgs#nodejs -c /bin/bash ./check.sh behaviour   # macOS
...
check_lint() {
  ...
  # The gate and the suite run under /bin/bash 3.2 in the macos workflow and claim it, and
  # have no dispatcher of a tool's shape, so the checker reads them by the proxy alone
  CHECK_SH_NESTED=1 ./check-sh.sh -n check.sh check.sh
  CHECK_SH_NESTED=1 ./check-sh.sh -n check-obsi.sh check-obsi.sh
  ...
  printf '%s\n' '# A document with measurements and no version' 'It returned 12 backlinks' \
    >"$work/versionless.md"
  # An if, not `grep && fail`: as the last command of this function a grep that finds
  # nothing would make the function's status 1, and set -e would end the gate there in silence
  if grep -qE "$version_claim" "$work/versionless.md"; then
...
check_behaviour() {
  ...
  # every run here. What stays measured rather than tested is the real app's index. Under
  # "$BASH", like every script it runs, so the bash asked is the one running the gate
  "$BASH" ./check-obsi.sh .
  ...
  if [[ -n "${CHECK_BASH32:-}" ]]; then
    ...
    mkdir -p "$work/bash4/tests"
    cp check-obsi.sh "$work/bash4/"
    cp -R tests/. "$work/bash4/tests/"
    awk '{ print } /^export -n out err$/ { print "declare -A obsi_probe" }' obsi.sh >"$work/bash4/obsi.sh"
    chmod +x "$work/bash4/obsi.sh"
    grep -q '^declare -A obsi_probe$' "$work/bash4/obsi.sh" ||
      fail "the declare -A was not planted — obsi.sh no longer has the line it goes after"
    if bash4_out=$("$BASH" "$work/bash4/check-obsi.sh" "$work/bash4" 2>&1); then
      fail "a copy of obsi.sh that declares an associative array passed under this bash — the suite does not ask the interpreter"
    fi
    [[ "$bash4_out" == *"-A: invalid option"* ]] ||
      fail "the copy with declare -A failed for another reason than this bash refusing it:"$'\n'"$bash4_out"
  fi
}
