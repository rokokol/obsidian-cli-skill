...
Teaches an agent to drive a vault through [Obsidian's official CLI](https://help.obsidian.md/cli) — reading notes, querying the link graph, searching, editing properties — without a plugin, an MCP server or a second copy of your vault. One `SKILL.md` and two reference files
...
The reference splits in two: [`references/commands.md`](references/commands.md) for output shape and counting semantics per command group, [`references/pitfalls.md`](references/pitfalls.md) for each trap with the reproduction that found it

## What it deliberately leaves out

- **The command list.** `obsidian-cli help` prints it, generated by the app, and the set is not fixed — `daily:*`, `web`, `workspaces`, `publish:*` and `sync:*` exist only when their core plugin or service is enabled. A copied list is a second source of truth that goes stale silently and disagrees with the machine it is read on
